...
Текущая версия: **0.1.2**. Изменения: [CHANGELOG.md](CHANGELOG.md).
...
Локальный помощник по текстовым PDF: Streamlit, ChromaDB, sentence-transformers, Ollama.
...
Окно запуска нужно оставить открытым во время работы. Для библиотеки: шестерёнка → «Внести изменения» → выбрать PDF → подтвердить → «Добавить PDF».
...
```powershell
git archive --format=zip --output=dist/yastreb-rag-v0.1.2-source.zip v0.1.2
```
